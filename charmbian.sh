...
DEBOOTSTRAP_DIR=usr/share/debootstrap usr/sbin/debootstrap --no-check-gpg --components=main,universe,restricted,multiverse --arch=armhf --foreign --include=alsa-utils,acpid,x11-xserver-utils,xserver-common,xserver-xorg,xserver-xorg-core,xserver-xorg-input-all,xserver-xorg-video-fbdev,links,gpicview,pcmanfm,xterm,ubuntu-mate-desktop,lightdm,xinit,usbutils,kmod,libkmod2,wget,curl,nmcli,network-manager,x11-utils,vim,pm-utils,mesa-utils focal /mnt
